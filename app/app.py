...
import db_connect as db  # Tu motor directo de base de datos
import joblib
import base64
import streamlit.components.v1 as components
import os
from langchain_ollama import OllamaLLM # La única herramienta que necesitamos para invocar tu modelo GGUF

# Carpeta donde vive este script: hace portables las rutas a imágenes/modelo
# (funcionan esté donde esté la carpeta, no atadas a E:\INGEDATOS).
...
def animacion_aviones_login():
    # 1. Leemos AMBAS imágenes locales
    # -- Imagen del Avión --
    try:
    ...
    # -- Imagen del Césped (Asegúrate de que sea un PNG sin fondo negro) --
    try:
    ...
    # 3. JavaScript: El Cerebro del Escape (Se mantiene igual)
    js = """
    <script>
    ...
    for c in df.columns:
        if pd.api.types.is_datetime64_any_dtype(df[c]):
            ...
            try:
                ...
                if parsed.notna().mean() > 0.8:          # ≥80 % parseables → es fecha
                    df[c] = parsed
                    cols_fecha.append(c)
                    cols_texto.remove(c)
            except Exception:
...
# --- 3. TU FUNCIÓN DE LOGIN MODIFICADA ---
def check_password():
    ...
    def password_entered():
        ...
        if st.session_state["username"] == "admin" and st.session_state["password"] == "minad123":
            ...
            del st.session_state["password"]  # Borramos la clave de la memoria
        else:
            st.session_state["password_correct"] = False

    if "password_correct" not in st.session_state:
        # PANTALLA DE INICIO (AQUÍ SÍ VAN LOS AVIONES)
        ...
        # LOGIN FALLIDO (AQUÍ TAMBIÉN VAN LOS AVIONES)
        animacion_aviones_login()
    ...
    # ELIMINAMOS LOS AVIONES Y APLICAMOS EL COLOR SÓLIDO
    estilo_dashboard()
    
    return True

# --- ENTORNO WEB (Solo visible si el login es correcto) ---
if check_password():
    # ── SIDEBAR: Header institucional ──
    # Logo institucional desde archivo local (la URL de Wikipedia dejó de responder).
    ...
    if menu == "Dashboard Operativo":
        ...
        with col_f1:
            # Barra deslizable para rango de meses (Min: 1, Max: 12, Valor por defecto: Rango completo 1-12)
        ...
        with col_f2:
            ...
            # Caja de selección múltiple (Mostramos 5 por defecto para no saturar el gráfico inicial)
        ...
        with st.spinner('Aplicando filtros y calculando métricas en SQL Server...'):
            ...
            # Query de VOLUMEN y CANCELADOS (incluye vuelos cancelados).
            # Los vuelos cancelados tienen ID_CR / ID_Programacion en NULL, por lo que
            # un INNER JOIN a CRONOMETRIA_REAL / DETALLE_RETRASOS los excluiría y el
            # conteo de cancelados siempre daría 0. Por eso aquí NO se usa la cadena de
            # demoras y se filtra el mes por MONTH(V.FlightDate), que sí existe en cancelados.
            query_kpi_volumen = f"""
            SELECT
            ...
            # Query de RETRASO PROMEDIO (solo vuelos NO cancelados, usando la cadena de demoras).
            query_kpi_retraso = f"""
            SELECT
            ...
            # Unificamos en un único df_kpi para mantener el resto del dashboard igual.
            df_kpi = df_volumen.copy()
            df_kpi['Retraso_Promedio'] = df_retraso['Retraso_Promedio'][0]

            # Query para el Gráfico (Corregido el JOIN path)
            query_grafico = f"""
            SELECT TOP 10
        ...
        # Creamos las pestañas para las 5 misiones
        ...
        with tab1:
            ...
            # ==========================================
            # CONSULTA BI-3 & A-2 (Combinada o en acordeón)
            # ==========================================
        ...
        with tab2:
            ...
            # CONSULTA BI-1
            # ==========================================
            with st.expander("Q1: Operaciones en Code-Share vs Propias", expanded=False):
                query_m2_bi1 = """
                WITH CodeShare AS (
                    SELECT
            ...
            # CONSULTA I-1
            # ==========================================
            with st.expander("Q2: Conectividad aérea por Estado", expanded=False):
                query_m2_i1 = """
            ...
            # CONSULTA I-2
            # ==========================================
            with st.expander("Q3: Actividad operativa de aerolíneas por trimestre", expanded=False):
                query_m2_i2 = """
                ...
                if not df_m2_i2.empty:
                    ...
                    # Pivotamos los datos (Melt) en Pandas para crear un gráfico de líneas trimestral
            ...
            # CONSULTA A-1
            # ==========================================
            with st.expander("Q4: Continentes con cobertura operativa completa", expanded=False):
                query_m2_a1 = """
            ...
            # CONSULTA A-2
            # ==========================================
            with st.expander("Q5: Rendimiento ambiental de rutas por modelo de avión", expanded=False):
                query_m2_a2 = """
        ...
        with tab3:
            ...
            with st.expander("Q11: Desvío de Tiempo Programado vs Real por Aerolínea", expanded=False):
                ...
                if not df_m3_q11.empty:
                    ...
                    # Limpiamos el formato de SQL para que Plotly pueda graficar los números
            ...
            with st.expander("Q13: Clasificación de Retraso según Percentiles", expanded=False):
                ...
                if not df_m3_q13.empty:
                    ...
                    # Gráfico de Dispersión (Scatter Plot) para comparar salida vs llegada
                    fig_q13 = px.scatter(
                        df_plot_q13, 
                        x='Salida_Num', 
                        y='Llegada_Num', 
        ...
        with tab4:
            st.info("El equipo de datos está preparando las consultas para la Misión 4...")
        with tab5:
            ...
            # CONSULTA M5-Q21
            # ==========================================
            with st.expander("Q21: Matriz Ejecutiva de Emisiones Trimestrales (CO2)", expanded=False):
                query_m5_q21 = """
                ...
                if not df_m5_q21.empty:
                    ...
                    # Preparación de datos para Plotly (quitando comas y convirtiendo a números)
            ...
            # CONSULTA M5-Q22
            # ==========================================
            with st.expander("Q22: Acumulación de Retrasos Year-To-Date (YTD)", expanded=False):
                query_m5_q22 = """
            ...
            # CONSULTA M5-Q23
            # ==========================================
            with st.expander("Q23: Gasto de Combustible Diario Month-To-Date (MTD)", expanded=False):
                query_m5_q23 = """
                WITH GastoDiario AS (
                    SELECT 
                ...
                if not df_m5_q23.empty:
                    ...
                    # Para graficar limpiamente, mostramos el comportamiento del mes más representativo (ej. Julio)
            ...
            # CONSULTA M5-Q24
            # ==========================================
            with st.expander("Q24: Matriz de Riesgo Operativo por Ruta Comercial", expanded=False):
                query_m5_q24 = """
            ...
            # CONSULTA M5-Q25
            # ==========================================
            with st.expander("Q25: Simulador de Rendimiento Parametrizable", expanded=True):
        ...
        # Solo aviones comerciales de ala fija: se excluyen helicópteros
        # (Turboshaft), avionetas (Piston) y registros sin clasificar (Unknown),
        # que el modelo no aprende bien por su escasa presencia en los datos.
        query_catalogo = """
        ...
        with col1:
            modelo_seleccionado = st.selectbox("Modelo de Aeronave (ICAO)", options=modelos_disponibles)
            distancia_vuelo = st.number_input("Distancia de la Ruta (Millas)", min_value=10, max_value=8000, value=1500)
            
        with col2:
            tiempo_estimado = st.number_input("Tiempo Estimado de Vuelo (Minutos)", min_value=10, max_value=1000, value=180)
            mes_vuelo = st.slider("Mes de Operación", min_value=1, max_value=12, value=6)

        st.markdown("---")

        # 3. Botón de Ejecución del Modelo
        if st.button("Calcular Proyección de Emisiones", type="primary", use_container_width=True):
        ...
        if not df_emisiones.empty:
            ...
            # 3. Gráfico Interactivo 1: Top 10 Modelos Contaminantes (Barras Horizontales)
            ...
            # 4. Gráfico Interactivo 2: Matriz de Eficiencia (Dispersión con Burbujas)
